...
- For usage examples, show consumers creating a `LivequeryClient` with a storage adapter and one or more transporters, then creating a `LivequeryCollection` and subscribing to reactive state.
- In generated app code, prefer one shared `LivequeryClient`, then create collections from it and call `initialize(ref)` before any query or mutation.
